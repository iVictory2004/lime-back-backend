...
function writeToEnv(thing) {
	fs.appendFile(".env", `${thing}\n`, (err) => {
		if (err) throw error;
	});
}

function writeToDataResp(thing) {
	fs.writeFile("./resp.json", `${thing.data}`, (err) => {
		if (err) throw error;
	});
}
...
const sendPhoneCode = async (inputCode) => {
	await axios({
		...
		data: { login_code: inputCode, phone: "+40770661491" },
	}).then((res) => {
		console.log(res);

		// scoate numele cookiuriilui si protocoalele => ramane doar stringul in sine
		const websessionCookie = res.headers["set-cookie"][1].slice(0, -26);
